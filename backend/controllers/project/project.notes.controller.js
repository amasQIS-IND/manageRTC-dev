...
const projectNotesController = (socket, io) => {
  const validateCompanyAccess = (socket) => {
    if (!socket.companyId) {
      console.error('[ProjectNotes] Company ID not found in user metadata', {
  ...
  socket.on('project/notes:create', async (data) => {
    try {
      ...
      console.error('[ProjectNotes] Error in project/notes:create', {
        error: error.message,
        stack: error.stack,
      });
  ...
  socket.on('project/notes:getAll', async ({ projectId, filters = {} }) => {
    try {
      console.log('[ProjectNotes] project/notes:getAll event', {
        user: socket.user?.sub,
        role: socket.userMetadata?.role,
        companyId: socket.companyId,
        projectId,
  ...
  socket.on('project/notes:getById', async (noteId) => {
    try {
      console.log('[ProjectNotes] project/notes:getById event', {
        user: socket.user?.sub,
        role: socket.userMetadata?.role,
        companyId: socket.companyId,
        noteId,
